...
drawimg(pole , ctx, width * .097,height*.65,10/width,height*.8/height);
drawimg(pole , ctx, width * .247,height*.65,10/width,height*.8/height);
drawimg(pole , ctx, width * .397,height*.65,10/width,height*.8/height);
...
drawimg(crowd, ctx_back_props, 0, height*3,1,1,.5);
    ...
    for (i = 0; i < width * .4; i += 12){
        for(j = height*.755; j> height * .655; j-- ){
            if(ctx.isPointInPath(i,j)){
                this.bulbs.push(new bulb_obj(i,cols[this.bulbs.length%6], i,j));
                break;
            }
        }
    }
...
Animation_cont_layer.prototype.update = function() {
    ...
    ctx4.clearRect(0, 0, canv_anim.width, canv_anim.height);
